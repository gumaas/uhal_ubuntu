...
DEFAULT_COMMANDS    = ["CLEANUP_WWW_AREA","UNINSTALL","ENVIRONMENT","DEPENDENCIES",
                       ...
                       "TEST_CENTRAL_CELL","TEST_RETRI_CELL","TEST_TTC",
...
def importCommands() :
  ...
  COMMANDS += [["DEPENDENCIES",
                ...
                "sudo cp ts13-beta.repo /etc/yum.repos.d/ts.repo",
                "sudo yum clean all && sudo yum clean metadata && sudo yum clean dbcache && sudo yum makecache",
                "sudo yum -y groupinstall triggersupervisor_core"
                ]]]
  ...
  COMMANDS += [["BUILD",
                [
                "cd %s;make -sk Set=tssub" % join(BUILD_HOME,"trunk"),
                "cd %s;make -sk Set=tssub rpm" % join(BUILD_HOME,"trunk")
                ]]]
  ...
  COMMANDS += [["INSTALL",
                ["sed \"s/<platform>/%s/\"  subsystem_dev.nightly.repo | sudo tee /etc/yum.repos.d/ts_subsystem.repo > /dev/null" % pseudo_platform,
                "sudo yum clean all && sudo yum clean metadata && sudo yum clean dbcache && sudo yum makecache",
                "sudo yum --disablerepo=cactus-base,cactus-updates -y groupinstall triggersupervisor_projects",
