...
def importCommands() :
  ...
  COMMANDS += [["DEPENDENCIES",
                ...
                "sudo yum clean all && sudo yum clean metadata && sudo yum clean dbcache && sudo yum makecache",
                "sudo yum -y groupinstall uhal triggersupervisor_core"
                ]]]
  ...
  COMMANDS += [["INSTALL",
                ["sed \"s/<platform>/%s/\"  ts_904.nightly.repo | sudo tee /etc/yum.repos.d/ts_subsystem.repo > /dev/null" % pseudo_platform,
                "sudo yum clean all && sudo yum clean metadata && sudo yum clean dbcache && sudo yum makecache",
                "sudo yum -y --disablerepo=ts-legacy-904 groupinstall gtgmt triggersupervisor_projects"]]]
